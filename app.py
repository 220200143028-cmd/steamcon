...
def answer_chat_question(question, knowledge_sentences):
    ...
    # Check if it's a construction-related question
    construction_keywords = [
        "construction", "building", "foundation", "concrete", "steel", "cement", 
        "project management", "blueprint", "architecture", "structure", "safety",
        "material", "labor", "cost", "timeline", "permit", "inspection"
    ]
    is_construction_query = any(keyword in question.lower() for keyword in construction_keywords)
    
    # If construction-related, search Google directly
    if is_construction_query:
        return search_google(question)
    
    # Otherwise, try to match with user knowledge
...
def search_google(query):
    """Search for information and return answer from web content."""
    try:
        headers = {
        ...
        if data.get("AbstractText"):
            ...
            if data.get("AbstractURL"):
                answer += f"\n\n**Source:** [{data.get('AbstractSource', 'Learn more')}]({data['AbstractURL']})"
            return answer
        
        # Get related topics if abstract not available
        if data.get("RelatedTopics") and len(data["RelatedTopics"]) > 0:
            topics = []
            for result in data["RelatedTopics"][:3]:  # Get top 3 results
                if "Text" in result:
                    text = result["Text"]
                    url_link = result.get("FirstURL", "")
                    if url_link:
                        topics.append(f"• {text[:150]}... [{url_link}]({url_link})")
                    else:
                        topics.append(f"• {text[:150]}...")
            if topics:
        ...
        # Fallback to Google search link on error
        search_url = f"https://www.google.com/search?q={urllib.parse.quote_plus(query)}"
        return f"🔍 **[Click here to search Google]({search_url})**"
...
with chat_tab:
    ...
    if st.session_state.selected_user is None:
        ...
        st.write(
            ...
            "- Construction topics (materials, techniques, safety, costs, etc.) - **Powered by Google Search**"
        )
